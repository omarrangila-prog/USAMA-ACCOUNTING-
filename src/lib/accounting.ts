...
export function computeStock(data: DataSet, period: Period): StockLine[] {
  ...
  return data.bondTypes.map((bt) => {
    ...
    // This month's own buying rate, with no carried stock blended in — so it
    // starts fresh each month and reads 0 when nothing was bought.
    const monthAvgCost = purchasedQty > 0 ? purchasedValue / purchasedQty : 0;
    ...
    return {
      ...
      monthAvgCost: round2(monthAvgCost),
...
export function computeBondMovement(data: DataSet, period: Period): BondMovement[] {
  return data.bondTypes.map((bt) => {
    ...
    const pVal = purchases.reduce((a, p) => a + p.amount, 0);
    // The rate BOUGHT AT this month — carried stock is deliberately not blended
    // in, so it resets each month like the Stock Report's Avg Cost column.
    const avgBuyRate = purchasedQty > 0 ? round2(pVal / purchasedQty) : 0;
    return {
      bondTypeId: bt.id,
      bondTypeName: bt.name,
      purchasedQty,
...
export function computeProfitLoss(data: DataSet, period: Period, cumulative = false): number {
  // Profit = trading profit (Sales − Cost of Sales), LESS this month's expenses
  // and PLUS this month's other income, minus any one-time Profit Closing dated
  // in the period.
  //
  // Expenses belong here rather than in Payables: an expense is money spent, not
  // money owed to a party, so it reduces Profit and never appears as a party
  // balance. (Recording an expense against a party — an "Exp" party — WOULD make
  // it a payable; enter it under Expenses instead.)
  //
  // The trading-profit formula itself is unchanged, and this stays the single
  // source of truth for the Profit figure on every screen and report.
  const trading = computeTradingProfit(data, period, cumulative);
  const { net } = computeExpenseNet(data, period);   // income − expense
  return round2(trading + net - profitClosingOffset(data, period, cumulative));
}
